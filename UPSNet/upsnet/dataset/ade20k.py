...
class ade20k(BaseDataset):
    ...
    def __init__(self, image_sets, flip=False, proposal_files=None, phase='train', result_path=''):
        ...
        image_dirs = {
            'places_training': os.path.join(config.dataset.dataset_path, 'images', 'places_training'),
            'frames_training': os.path.join(config.dataset.dataset_path, 'images', 'frames_training'),
            'places_validation': os.path.join(config.dataset.dataset_path, 'images', 'places_validation'),
        }

        anno_files = {
            'places_training': 'instances_places_training_gts.json',
            'frames_training': 'instances_frames_training_gts.json',
            'places_validation': 'instances_places_validation_gts.json',
        }
    ...
    def __getitem__(self, index):
        ...
        if config.network.has_fcn_head:
            if self.phase != 'test':
                seg_gt = np.array(Image.open(self.roidb[index]['image'].replace('images', 'annotations').replace('.jpg', '.png')))
                ...
                for i in range(len(label['gt_classes'])):
                    if type(label['roidb']['segms'][i]) is list and type(label['roidb']['segms'][i][0]) is list:
                        ...
                        if type(label['roidb']['segms'][i]) is dict:
                            label['mask_gt'][i] = cv2.resize(mask_util.decode(
                                mask_util.frPyObjects([label['roidb']['segms'][i]],
                                                      label['roidb']['segms'][i]['size'][0],
                                                      label['roidb']['segms'][i]['size'][1]))[:, :, 0], None, None,
